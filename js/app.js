...
const ItemVenda_1 = require("../models/ItemVenda");
 ...
 */
function carregarDadosIniciais() {
    clientes = DataService_1.DataService.carregarClientes();
    produtos = DataService_1.DataService.carregarProdutos();
    vendas = DataService_1.DataService.carregarVendas();
    ...
    $('#form-cliente').on('submit', function (event) {
        ...
        try {
            if (id) {
                ...
                // Modo cadastro
                const novoCliente = new Cliente_1.Cliente(nome, cpf, email, telefone);
                clientes.push(novoCliente);
                exibirMensagem('Cliente cadastrado com sucesso!', 'success');
            }
            DataService_1.DataService.salvarClientes(clientes);
            renderizarTabelaClientes();
...
function inicializarProdutosPage() {
    ...
    $('#form-produto').on('submit', function (event) {
        ...
        try {
            if (id) {
                ...
                // Modo cadastro
                const novoProduto = new Produto_1.Produto(nome, preco, estoque);
                produtos.push(novoProduto);
                exibirMensagem('Produto cadastrado com sucesso!', 'success');
            }
            DataService_1.DataService.salvarProdutos(produtos);
            renderizarTabelaProdutos();
...
function inicializarVendasPage() {
    ...
    $('#btn-adicionar-produto').on('click', function () {
        ...
        const quantidade = parseInt($('#quantidade-produto').val());
        ...
        try {
            if (itemExistente) {
                ...
                const novoItem = new ItemVenda_1.ItemVenda(produto, quantidade);
                itensDaVenda.push(novoItem);
            }
            // Reduz o estoque do produto
            produto.estoque -= quantidade;
            DataService_1.DataService.salvarProdutos(produtos); // Salva o estoque atualizado
    ...
    $('#lista-itens-venda').on('click', '.btn-remove-item', function () {
        ...
        if (confirm('Tem certeza que deseja remover este item da venda?')) {
            ...
            renderizarItensDaVenda();
            exibirMensagem('Item removido da venda.', 'info');
        }
    });
    $('#btn-finalizar-venda').on('click', function () {
        ...
        try {
            const novaVenda = new Venda_1.Venda(clienteSelecionado, [...itensDaVenda]); // Cria uma cópia dos itens
            vendas.push(novaVenda);
            DataService_1.DataService.salvarVendas(vendas);
